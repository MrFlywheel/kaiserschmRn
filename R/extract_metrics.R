extract_metrics <- function(data_path, type=NULL, date = NULL, polygons=NULL, stepsize = 0.1, out_path = NULL, PC_type = NULL, n_cores = NULL, parallel = TRUE){
  ...
  VI.name <- function(name, date){
    return(c(paste(date,  'NDVI', name, sep = "_"), paste(date,  'GNDVI', name, sep = "_"),
             paste(date,  'RENDVI', name, sep = "_"), paste(date,  'NDWI', name, sep = "_"),
             paste(date,  'CVI', name, sep = "_"), paste(date,  'EVI', name, sep = "_"),
             paste(date,  'CCCI', name, sep = "_"), paste(date,  'SAVI', name, sep = "_"),
             paste(date,  'NormG', name, sep = "_"), paste(date,  'VDVI', name, sep = "_")))
  ...
  get.lidar.metrics.RGB <- function(data, stepsize, date, PC_type=PC_type){

    stopifnot("stepsize must be between 0 and 1" = stepsize > 0 & stepsize < 1)
    stopifnot("'PC_type' must be equal to 1 (for .txt file) or 2 (for .las file from LiDAR) or 3 (for .las files from photogrammetry" = !is.null(PC_type))
    ...
    if(PC_type==3){
      df <- list()
      df$NDVI <- (data$B5 - data$R) / (data$B5 + data$R)
      df$NDVI <- kaiserschmRn::remove_Inf_Nan(df$NDVI)
      df$GNDVI <- (data$B5 - data$G) / (data$B5 + data$G)  # Gitelson et al. (1996)
      df$GNDVI <- kaiserschmRn::remove_Inf_Nan(df$GNDVI)
      df$RENDVI <- (data$B5 - data$B4) / (data$B5 + data$B4)
      df$RENDVI <- kaiserschmRn::remove_Inf_Nan(df$RENDVI)
      df$NDWI <- (data$G - data$B5) / (data$G + data$B5)
      df$NDWI <- kaiserschmRn::remove_Inf_Nan(df$NDWI)
      df$CVI <- (data$B5 * (data$R / (data$G ** 2)))
      df$CVI <- kaiserschmRn::remove_Inf_Nan(df$CVI)
      df$EVI <- (2.5 * ((data$B5 - data$R) / ((data$B5 + 6* data$R - 7.5 * data$B) + 1)))
      df$EVI <- kaiserschmRn::remove_Inf_Nan(df$EVI)
      df$CCCI <- (((data$B5 - data$B4) / (data$B5 + data$B4)) / ((data$B5 - data$R) / (data$B5 + data$R)))
      df$CCCI <- kaiserschmRn::remove_Inf_Nan(df$CCCI)
      df$SAVI <- (1 + 0.5)*(data$B5-data$R)/(data$B5 + data$R + 0.5) # Huete (1988) - Soil adjusted vegetation index
      df$SAVI <- kaiserschmRn::remove_Inf_Nan(df$SAVI)
      df$NormG <- data$G / (data$B + data$G + data$R)
      df$NormG <- kaiserschmRn::remove_Inf_Nan(df$NormG)
      df$VDVI <- (2 * data$G - data$R - data$B) /  (2 * data$G + data$R + data$B)# Huete (1988) - Soil adjusted vegetation index
      df$VDVI <- kaiserschmRn::remove_Inf_Nan(df$VDVI)

      df.means <- matrix(lapply(df, FUN=mean, na.rm = T), ncol=length(df), byrow = T)
      colnames(df.means) <- VI.name('mean', date)
      df.sd <- matrix(lapply(df, FUN=sd, na.rm = T), ncol=length(df), byrow = T)
      colnames(df.sd) <- VI.name('sd', date)
      df.var <- matrix(lapply(df, FUN=var, na.rm = T), ncol=length(df), byrow = T)
      colnames(df.var) <- VI.name('var', date)
      df.skew <- matrix(lapply(df, FUN= skewness, na.rm = T), ncol=length(df), byrow = T)
      colnames(df.skew) <- VI.name('skew', date)
      df.kurt <- matrix(lapply(df, FUN= kurtosis, na.rm = T), ncol=length(df), byrow = T)
      colnames(df.kurt) <- VI.name('kurt', date)
      df.MAD <- matrix(lapply(df, FUN= mad, na.rm = T), ncol=length(df), byrow = T)
      colnames(df.MAD) <- VI.name('MAD', date)
      df.IQR <- matrix(lapply(df, FUN= IQR, na.rm = T), ncol=length(df), byrow = T)
      colnames(df.IQR) <- VI.name('IQR', date)


      df.q90 <- matrix(unlist(lapply(df, quantile, probs = c(0.90), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q90) <- VI.name('q90', date)
      df.q80 <- matrix(unlist(lapply(df, quantile, probs = c(0.80), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q80) <- VI.name('q80', date)
      df.q70 <- matrix(unlist(lapply(df, quantile, probs = c(0.70), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q70) <- VI.name('q70', date)
      df.q60 <- matrix(unlist(lapply(df, quantile, probs = c(0.60), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q60) <- VI.name('q60', date)
      df.q50 <- matrix(unlist(lapply(df, quantile, probs = c(0.50), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q50) <- VI.name('q50', date)
      df.q40 <- matrix(unlist(lapply(df, quantile, probs = c(0.40), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q40) <- VI.name('q40', date)
      df.q30 <- matrix(unlist(lapply(df, quantile, probs = c(0.30), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q30) <- VI.name('q30', date)
      df.q20 <- matrix(unlist(lapply(df, quantile, probs = c(0.20), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q20) <- VI.name('q20', date)
      df.q10 <- matrix(unlist(lapply(df, quantile, probs = c(0.10), na.rm = T)), ncol=length(df), byrow = T)
      colnames(df.q10) <- VI.name('q10', date)
    ...
    if(PC_type==3){
      return(data.frame(cbind(df.means, df.sd, df.var, df.skew, df.kurt, df.q90, df.q80,
                              df.q70, df.q60, df.q50, df.q40, df.q30, df.q20, df.q10, df.IQR,
                              z_mean, z_sd, z_var, z_skew, z_kurt, z_q, z_iqr, z_Dens, z_VCI)))
    }
  }
    ...
    for (i1 in seq_along(data_path)){
      ...
      if(parallel){
        ...
        result <- foreach(i2 = seq_along(pc_files), .combine = rbind, .packages = c("pizzR", "lidR", 'moments')) %dopar% {
          ...
          lidar_metrics <- get.lidar.metrics.RGB(data, stepsize=0.1, PC_type = PC_type, date = date[i1])
          gc(reset = T, full = T)
          lidar_metrics #gc must not be last field, as it would be written to 'result'
        }
        stopCluster(cl)
        result2 <- matrix(unlist(result), ncol=length(result), byrow=T)
        result2 <- cbind(as.numeric(seg_ID), result2)
        colnames(result2) <- c('Segment_ID', names(result))
        #stopifnot("las naming contains characters. Should only be named with the segment ID as number in order to write it in the extract table."=typeof(seg_ID)!='character')
        rownames(result2) <- seg_ID
        write.csv2(result2, paste0(date[i1],'_LiDAR_metrics.csv'))
      }
      else{
        if(is.null(out_path)) out_path <- file.path(dirname(data_path), '/export')
        pizzR::setcreate.wd(out_path)
  ...
  print(paste0('Extract done! Data written to disk. ', out_path))
  pizzR::Systime()
}
